...
class NETTrainer(Trainer):
    """
    NET Trainer, train the NET with specified hyperparameters and configurations
    """
    def __init__(self,
                 trainset,
                 evalset,
        ...
        if self.multi_gpu:
            ...
            if self.verbose and (not self.multi_gpu or (self.multi_gpu and self.cuda_id == 1)):
                print("[NETTrainer]: Train the mode with multiple GPUs: {} GPUs.".format(int(os.environ['WORLD_SIZE'])))
        else:
            if self.verbose and (not self.multi_gpu or (self.multi_gpu and self.cuda_id == 1)):
                print("[NETTrainer]: Train the mode with single device on {}.".format(self.device))
    ...
    def compute_loss(self, data):
        n = data.candidate_len_max[0] # 最大的candidate个数
        data.y = data.y.view(-1, self.horizon, 2).cumsum(axis=1) # [batch_size, horizon, 2]
    ...
    def test(self,
        ...
        if compute_metric:
            ...
            print("[NETTrainer]: The test result: {};".format(metric_results))
