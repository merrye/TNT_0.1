...
class VectorNetBackbone(nn.Module):
    ...
    def forward(self, data):
        ...
        sub_graph_out = self.subgraph(data) # [num_nodes / num_polylines, subgraph_width]

        if self.training and self.with_aux:
            randoms = 1 + torch.rand((batch_size,), device=self.device) * (valid_lens - 2) + \
                      time_step_len * torch.arange(batch_size, device=self.device) # [batch_size, ]
        ...
        """
            sub_graph_out -> [num_nodes / num_polylines, subgraph_width]
            id_embedding  -> [num_nodes / num_polylines, 2]
            x             -> [batch_size, time_step_len, subgraph_width + 2]
            num_nodes = batch_size * time_step_len
        """
        x = torch.cat([sub_graph_out, id_embedding], dim=1).view(batch_size, -1, self.subgraph.out_channels + 2)
        valid_lens = data.valid_len # [batch_size]
        ...
        if self.training:
            ...
            # global_graph_out.shape (batch_size, time_step_len, global_graph_width)
            global_graph_out = self.global_graph(x, valid_lens=valid_lens)

            if self.with_aux:
                # aux_in.shape (batch_size, global_graph_width)
                aux_in = global_graph_out.view(-1, self.global_graph_width)[mask_polyline_indices]
                # aux_out.shape (batch_size, global_graph_width)
...
if __name__ == "__main__":
    ...
    INTERMEDIATE_DATA_DIR = "~/Desktop/TNT/dataset/interm_data_small"
