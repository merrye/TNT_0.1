...
class GlobalGraph(nn.Module):
    ...
    def forward(self, x, **kwargs):
        # x.shape (batch_size, time_step_len, global_graph_width + 2)
        for name, layer in self.layers.named_modules():
            if isinstance(layer, SelfAttentionFCLayer):
                x = layer(x, **kwargs)
        # x.shape (batch_size, time_step_len, global_graph_width)
        return x
...
class SelfAttentionFCLayer(nn.Module):
    ...
    def forward(self, x, valid_lens):
        query = self.q_lin(x) # [batch_size, time_step_len, global_graph_width]
        key = self.k_lin(x)   # [batch_size, time_step_len, global_graph_width]
        value = self.v_lin(x) # [batch_size, time_step_len, global_graph_width]

        """
            torch.bmm -> 矩阵乘法： (B, N , M) * (B, M, P) = (B, N, P)
            query              : batch_size * time_step_len * global_graph_width
            key.transpose(1, 2): batch_size * global_graph_width * time_step_len
            scores             : batch_size * time_step_len * time_step_len
        """
        scores = torch.bmm(query, key.transpose(1, 2)) / math.sqrt(self.graph_width) # [batch_size, time_step_len, time_step_len]
        attention_weights = self.masked_softmax(scores, valid_lens) # [batch_size, time_step_len, time_step_len]
        x = torch.bmm(attention_weights, value)  # [batch_size, time_step_len, global_graph_width]
        return x
